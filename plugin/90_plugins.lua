...
end, { desc = 'Add cursor by regex' })

vim.keymap.set('x', '<M-s>', function()
  require('multicursor-nvim').splitCursors('^')
end, { desc = 'Split on newlines' })
...
vim.keymap.set({ 'n', 'o', 'x' }, 'd', function()
...
vim.api.nvim_create_autocmd({ 'BufReadPost', 'BufNewFile' }, {
  ...
  callback = function(ev)
    ...
    -- 跳过特殊 buffer
    if buftype ~= '' or filetype == '' or filetype == 'qf' or filetype == 'help' then
      return
    end
...
wk.add({
  ...
  { '<leader>w', group = '+Window', expand = function()
